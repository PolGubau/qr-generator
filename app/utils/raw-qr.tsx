namespace qrcodegen {
	...
	export class QrCode {
		...
		private finderPenaltyTerminateAndCount(
			...
			currentRunLengthProp: int,
			runHistory: Array<int>,
		): int {
			let currentRunLength = currentRunLengthProp;
			if (currentRunColor) {
				// Terminate dark run
		...
		private finderPenaltyAddHistory(len: int, runHistory: Array<int>): void {
			let currentRunLength = len;
			if (runHistory[0] === 0) {
	...
	export class QrSegment {
		...
		public static makeSegments(text: string): Array<QrSegment> {
			...
			const options = [
				...
				{
					test: QrSegment.isNumeric,
					make: QrSegment.makeNumeric,
				},
				{
					test: QrSegment.isAlphanumeric,
					make: QrSegment.makeAlphanumeric,
				},
			];
		...
		private static toUtf8ByteArray(strProp: string): Array<byte> {
			const str = encodeURI(strProp);
			const result: Array<byte> = [];
